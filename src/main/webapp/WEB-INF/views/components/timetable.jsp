...
<div class="grid grid-flow-row grid-cols-8 auto-cols-max gap-1">
  ...
  <c:set var="timeSlotsString" value="7:30 - 9:50,10:00 - 12:20,12:50 - 15:10,15:20 - 17:40,17:50 - 20:10,20:20 - 22:40"/>
  ...
  <c:forEach items="${requestScope.weeklySchedule}" var="sessions" varStatus="loop">
    ...
    <c:forEach items="${sessions}" var="session">
      <c:choose>
        ...
        <c:otherwise>
          <div class="${!session.isYetToStart() ? "bg-yellow-100" : session.getStatus().get(0).isAttended() ? "bg-green-200" : "bg-red-200"} p-2 text-center text-slate-600 font-bold">
            <a href="#">${session.getGroup().getCourse().getName()}</a>
            <br>
